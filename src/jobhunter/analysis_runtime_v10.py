"""Runtime wiring for the isolated English P1.6 v10 candidate."""
...
from jobhunter.analysis_runtime import (
    RuntimeManagedAnalysisProvider,
...
class V10CandidateAnalysisProvider(RuntimeManagedAnalysisProvider):
    """Add one bounded v10 semantic correction above the existing Instructor path."""
    ...
    def complete_structured(self, **kwargs: Any) -> StructuredInferenceResult:
        ...
        if not isinstance(analysis_fields, dict):
            raise AnalysisValidationError("P1.6 v10 runtime is missing analysis_fields")
        ...
        try:
            ...
            validate_v10_candidate_structured(corrected.structured, analysis_fields)
            request_body = dict(corrected.request_body)
            runtime_payload = dict(request_body.get("runtime") or {})
            runtime_payload["p16_v10_candidate_recovery"] = True
            runtime_payload["p16_v10_first_error"] = str(first_error)
            request_body["runtime"] = runtime_payload
            return replace(corrected, request_body=request_body)


def build_v10_candidate_analysis_service(settings: Settings) -> JobAnalysisServiceV10:
    """Build the isolated v10 candidate without changing production v9 orchestration."""
...
__all__ = ["V10CandidateAnalysisProvider", "build_v10_candidate_analysis_service"]
