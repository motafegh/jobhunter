"""Blueprint v6 contracts with no free-form role-summary inference surface."""
...
def _uses_unbounded_inference_language(value: str) -> bool:
    ...
    return bool(
        ...
        or _FULL_SCOPE_RE.search(cleaned)
    )
...
# ---------------------------------------------------------------------------
# Model-facing draft. Every generated positive statement carries uncertainty.
# ---------------------------------------------------------------------------
...
class BlueprintProfessionalConsiderationDraft(_StrictModel):
    ...
    def consideration_must_remain_bounded(
        self,
    ) -> BlueprintProfessionalConsiderationDraft:
        if _uses_unbounded_inference_language(self.statement):
            raise ValueError(
                "Professional considerations cannot claim employer obligation or full scope"
            )
        return self
...
class BlueprintCapabilityInterpretationDraft(_StrictModel):
    ...
    important_unknowns: list[str] = Field(min_length=1, max_length=8)


class RoleBlueprintDraft(_StrictModel):
    """LLM-owned v6 draft: bounded considerations and unknowns only."""
...
# ---------------------------------------------------------------------------
# Persisted v6 Blueprint. Source facts remain JobHunter-owned.
# ---------------------------------------------------------------------------
...
class RoleCapabilityBlueprint(_StrictModel):
    """Persisted v6 Blueprint: source truth plus explicitly uncertain inference."""
...
def reconcile_role_blueprint_v6(
    ...
    *,
    ...
    if not isinstance(source_truth, dict):
        raise ValueError("Blueprint v6 requires accepted Capability v7 source_truth")
    if not capability_profiles:
        raise ValueError("Blueprint v6 requires at least one accepted Capability profile")
    if len(draft.capability_interpretations) != len(capability_profiles):
        raise ValueError(
            "Blueprint v6 requires exactly one interpretation per accepted Capability profile "
