...
settings = Settings.load("jobhunter.toml").model_copy(
    update={"database_path": out / "sandbox.sqlite3"}
)
market = MarketStore(settings.database_path)
first = market.get_definition_version(1)
intent = (
    "Applied AI / ML engineering where the primary work is developing, evaluating, or "
...
definition = market.create_definition_version(
    first.target_market_id,
    spec=replace(first.spec, membership_intent=intent),
...
class Recorder:
    ...
    def classify(self, payload):
        self.calls += 1
        (out / f"{self.job}-clarified-input.json").write_text(
            json.dumps(payload, ensure_ascii=False, indent=2)
        )
        decision = provider.classify(payload)
        (out / f"{self.job}-clarified-decision.json").write_text(
...
report = {
    "base_head": base["head"],
    "model": base["model"],
    "comparison": "only target definition changed; same model/prompt/evidence",
