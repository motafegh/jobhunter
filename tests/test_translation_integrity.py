...
from jobhunter.translation.integrity import audit_translation_integrity, require_translation_integrity
...
def test_translation_integrity_accepts_sane_projection() -> None:
    ...
    projection = build_english_projection(
        ...
        translations={
            ...
            "تمام وقت": "Full-time",
            "توافقی": "Negotiable",
            "توسعه مدل‌های هوش مصنوعی با Python": "Develop artificial intelligence models with Python",
        },
    )
...
def test_translation_integrity_rejects_company_description_in_company_field() -> None:
    ...
    projection = build_english_projection(
        ...
        translations={
            "رادهوش | Radhoosh": (
                ...
                "continued building many software products for customers across several markets."
            ),
    ...
    assert any(item.code == "suspicious_scalar_expansion" for item in report.findings)
...
def test_translation_integrity_rejects_changed_date() -> None:
    ...
    projection = build_english_projection(
        source,
        translations={"مهندس امنیت": "Security Engineer", "تحلیل امنیت": "Security analysis"},
    )
