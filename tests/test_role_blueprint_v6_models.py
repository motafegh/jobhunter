...
from jobhunter.role_blueprint_v6_models import (
    RoleBlueprintDraft,
    reconcile_role_blueprint_v6,
)
...
def test_v6_reconciliation_keeps_source_truth_separate_from_inference() -> None:
    draft = RoleBlueprintDraft.model_validate(_draft_payload())
    result = reconcile_role_blueprint_v6(
        draft,
    ...
    assert result.capability_areas[0].important_unknowns == ["The serving topology is not stated."]


def test_v6_requires_one_interpretation_per_accepted_capability() -> None:
...
def test_v6_requires_an_unknown_per_capability() -> None:
    payload = _draft_payload()
    payload["capability_interpretations"][0]["important_unknowns"] = []

    with pytest.raises(ValidationError):
        RoleBlueprintDraft.model_validate(payload)


def test_v6_rejects_obligation_and_full_scope_language() -> None:
    payload = _draft_payload()
    ...
    with pytest.raises(ValidationError, match="cannot claim employer obligation or full scope"):
        RoleBlueprintDraft.model_validate(payload)

    payload = _draft_payload()
    consideration = payload["capability_interpretations"][0]["professional_considerations"][0]
    consideration["statement"] = "A full end-to-end infrastructure is likely."
    with pytest.raises(ValidationError, match="cannot claim employer obligation or full scope"):
        RoleBlueprintDraft.model_validate(payload)


def test_model_facing_v6_schema_removes_free_form_role_summary_surface() -> None:
    ...
    assert "important_unknowns" in serialized
