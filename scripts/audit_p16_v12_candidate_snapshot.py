#!/usr/bin/env python3
"""Mechanical audit for the isolated English P1.6 v12 candidate snapshot."""
...
from jobhunter.analysis_service_v12 import ENGLISH_PROMPT_VERSION, qualification_list_spans
...
def audit_snapshot(path: Path, *, job_id: str) -> None:
    ...
    _require(
        ...
        "English projection does not match P1.6 v12 candidate",
    )
    ...
    _require(
        ...
        "P1.6 v12 candidate references a different English projection",
    )
    ...
    _require(bool(coarse_decomposed), "No coarse requirement coverage was marked decomposed")

    print("Mechanical P1.6 v12 candidate checks: PASS")
    ...
    print(f"Decomposed coarse coverage decisions: {len(coarse_decomposed)}")
    print(f"Coverage decisions: {len(coverage)}")


def main() -> int:
    parser = argparse.ArgumentParser(
        description="Audit one isolated English P1.6 v12 candidate snapshot."
    )
    ...
    try:
        ...
        print(f"Mechanical P1.6 v12 candidate checks: FAIL: {exc}")
        return 1
    return 0
