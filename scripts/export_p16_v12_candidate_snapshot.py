#!/usr/bin/env python3
"""Export a normal review snapshot with the isolated English P1.6 v12 candidate selected."""
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Export one repository-reviewable snapshot using English P1.6 v12 candidate."
    )
    ...
    if candidate is None:
        raise SystemExit(
            ...
            f"{args.job_id!r}; run scripts/run_p16_v12_candidate.py first"
        )
    ...
    if translation is None:
        raise SystemExit("P1.6 v12 candidate has no resolvable English projection dependency")
