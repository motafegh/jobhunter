...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Run isolated guarded Capability v9 reasoning for one current job."
    )
    ...
    try:
        ...
        result = build_capability_v9_candidate_service(settings).analyze_job(args.job_id)
        ...
        if artifact is None:
            raise RuntimeError("Capability v9 artifact is unavailable after successful analysis")
    except (ConfigLoadError, CapabilityIntelligenceError, ValueError) as exc:
        print(f"Capability v9 candidate is not ready: {exc}", file=sys.stderr)
        return 2
    except (InferenceProviderError, OSError, RuntimeError) as exc:
        print(f"Capability v9 candidate failed: {exc}", file=sys.stderr)
        return 1

    print(f"Outcome: {result.outcome}")
    print(f"Artifact: {result.artifact_id}")
    print(format_capability_v9(artifact))
    return 0
