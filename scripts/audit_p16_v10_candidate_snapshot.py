#!/usr/bin/env python3
"""Mechanical audit for the isolated English P1.6 v10 candidate snapshot."""
...
from jobhunter.analysis_service_v10 import ENGLISH_PROMPT_VERSION
...
def audit_snapshot(path: Path, *, job_id: str) -> None:
    ...
    _require(
        ...
        "English projection does not match P1.6 v10 candidate",
    )
    ...
    _require(
        ...
        "P1.6 v10 candidate references a different English projection",
    )
    ...
    for skill in structured_skills:
        ...
        _require(
            any(
                ...
                and _normalize(str(item.get("evidence") or "")) == _normalize(skill)
                and item.get("disposition") == "extracted_requirement"
                for item in coverage
            ),
            f"Structured skill is missing deterministic coverage accounting: {skill!r}",
        )
    ...
    for index, item in enumerate(responsibilities):
        ...
        _require(
            ...
            f"responsibility[{index}] is grounded in qualification wording",
        )

    print("Mechanical P1.6 v10 candidate checks: PASS")
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Audit one isolated English P1.6 v10 candidate snapshot."
    )
    ...
    try:
        ...
        print(f"Mechanical P1.6 v10 candidate checks: FAIL: {exc}")
        return 1
    return 0
