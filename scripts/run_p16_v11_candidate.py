#!/usr/bin/env python3
"""Run the isolated English P1.6 v11 candidate for one explicit job."""
...
from jobhunter.analysis_runtime_v11 import build_v11_candidate_analysis_service
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Run isolated English P1.6 v11 candidate analysis for one current job."
    )
    ...
    try:
        ...
        service = build_v11_candidate_analysis_service(settings)
        result = service.analyze_english_job(args.job_id)
    except (ConfigLoadError, AnalysisValidationError, ValueError) as exc:
        print(f"P1.6 v11 candidate is not ready: {exc}", file=sys.stderr)
        return 2
    except (InferenceProviderError, OSError, RuntimeError) as exc:
        print(f"P1.6 v11 candidate failed: {exc}", file=sys.stderr)
        return 1

    print(f"Outcome: {result.outcome}")
    print(f"English P1.6 v11 candidate for {result.source_job_id}")
