#!/usr/bin/env python3
"""Export a normal review snapshot with the isolated English P1.6 v10 candidate selected."""
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Export one repository-reviewable snapshot using English P1.6 v10 candidate."
    )
    ...
    if candidate is None:
        raise SystemExit(
            ...
            f"{args.job_id!r}; run scripts/run_p16_v10_candidate.py first"
        )
    ...
    if translation is None:
        raise SystemExit("P1.6 v10 candidate has no resolvable English projection dependency")

    snapshot["english_analysis"] = _analysis_payload(candidate)
    snapshot["english_projection"] = _translation_payload(translation)
    # Existing Capability/Blueprint artifacts were built above another P1.6 identity and are
    # deliberately excluded from this candidate snapshot until rebuilt against v10.
