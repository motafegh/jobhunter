"""Audit the bounded Blueprint v6 review snapshot used for B4 acceptance."""
...
EXPECTED_BLUEPRINT_PROMPT = "role-capability-blueprint-v6"
EXPECTED_BLUEPRINT_SCHEMA = "role-capability-blueprint-v5"
DEFAULT_JOB_ID = "tG9K"
...
_FULL_SCOPE_RE = re.compile(
    r"\b(?:entire|full|end-to-end)\s+(?:lifecycle|stack|pipeline|system|infrastructure)\b",
    re.I,
)
...
class AuditError(RuntimeError):
    """Raised when a mechanically checkable Blueprint v6 invariant fails."""
...
def _uses_unbounded_inference_language(value: str) -> bool:
    cleaned = _NEGATED_REQUIREMENT_RE.sub("", value)
    return bool(_ABSOLUTE_REQUIREMENT_RE.search(cleaned) or _FULL_SCOPE_RE.search(cleaned))
    ...
    if isinstance(value, dict):
        for key, item in value.items():
            _require(
                ...
                f"legacy Blueprint key remains at {path}.{key}",
            )
...
def audit_snapshot(path: Path, *, job_id: str = DEFAULT_JOB_ID) -> dict[str, int]:
    ...
    for capability_index, (area, profile) in enumerate(
        ...
        for key in _FORBIDDEN_AREA_KEYS:
            _require(key not in area, f"v6 area unexpectedly contains {key}")
        ...
        area_unknowns = list(area.get("important_unknowns") or [])
        _require(area_unknowns, f"Capability {capability_index} has no important unknowns")
        for item in area_unknowns:
            _require(bool(str(item).strip()), "empty important unknown")
        unknowns += len(area_unknowns)
        ...
        for item in area.get("professional_considerations") or []:
            ...
            _require(
                not _uses_unbounded_inference_language(statement),
                "professional consideration claims employer obligation/full scope",
            )
            considerations += 1
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Audit the bounded Blueprint v6 review snapshot")
...
def main() -> int:
    ...
    try:
        ...
        print(f"Mechanical Blueprint v6 checks: FAIL\n{exc}")
        return 1

    print("Mechanical Blueprint v6 checks: PASS")
    ...
    print(f"Important unknowns: {result['important_unknowns']}")
